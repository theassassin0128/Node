...
module.exports = {
  ...
  async execute(client, interaction, lng) {
    ...
    const reason =
      options.getString("reason") ?? t("commands:ban.reason", { lng });
    const bot = await guild.members.fetchMe();

    if (!target) errArray.push(t("commands:ban.errors.member", { lng }));
    if (!target.moderatable)
      errArray.push(t("commands:ban.errors.moderatable", { lng }));
    if (!target.manageable)
      errArray.push(t("commands:ban.errors.manageable", { lng }));
