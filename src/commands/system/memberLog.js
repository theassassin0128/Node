	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((option) =>
			option
				...
				.addRoleOption((option) =>
					option.setName("bot_role").setDescription("Set a role to give to new bots"),
				),
		)
		.addSubcommand((option) =>
			option.setName("delete").setDescription("Delete the member logging system."),
		)
		.addSubcommand((option) =>
			option.setName("toggle").setDescription("Toggle the member logging system."),
		)
		.addSubcommand((option) =>
			option
				.setName("edit-role")
		...
		.addSubcommand((option) =>
			option
				...
				.setDescription("Edit the log channel of the member logging system.")
			...
			case "setup":
				{
					...
					const embed = new EmbedBuilder()
						...
						.setDescription(
							[
								...
								`- Bot Auto Role : ${botRole ? `<@&${botRole}>` : `None Specified`}`,
							].join("\n"),
						);
					...
					if (!doc) {
						...
						interaction.reply({
							content: "**Finished setting up the member logging system.**",
							embeds: [embed],
						});
					} else {
						interaction.reply({
							content: "**This Server already has a member logging system**.",
							ephemeral: true,
						});
					}
				}
			...
			case "delete":
				{
					...
					if (!doc) {
						interaction.reply({
							content: "**This server doesn't have a member-log system**",
							ephemeral: true,
						});
					} else {
						await memberLog.deleteOne({ Guild: guildId });
						interaction.reply({
							content: "**Successfully deleted member-log system.**",
							ephemeral: true,
						});
					}
				}
			...
			case "toggle":
				{
					...
					if (!doc) {
						interaction.reply({
							content: "This server does not have a member logging system.",
							ephemeral: true,
						});
					} else {
						...
						if (toggle == "ON") {
							...
							doc.save();
							interaction.reply({
								content: "Turned **OFF** the member logging system.",
								ephemeral: true,
							});
						} else {
							doc.$set("Toggle", "ON");
							doc.save();
							interaction.reply({
								content: "Turned **ON** the member logging system.",
								ephemeral: true,
							});
						}
					}
			...
			case "preview":
				{
					...
					let channel = (await guild.channels.fetch()).get(doc.get("Channel"));
					if (!channel) return;
					...
					if (risk == "High" || risk == "Extreme") {
						...
						const Buttons = new ActionRowBuilder().addComponents(kickButton, banButton);
