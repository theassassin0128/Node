 ...
 * @param {import("@types/utils").OldCommand} OldCommand
 * @param {import("@types/command").CommandStructure} NewCommand
 * @returns {boolean}
 */
	...
	if (OldCommand.global || (!NewCommand.testOnly ?? false)) {
		...
		if (oldCommand.integrationTypes || newCommand.integration_types) {
			if (!Array.isArray(newCommand.integration_types)) {
				if (oldCommand.integrationTypes.map((i) => i).join("") !== "0") return true;
			}
...
function checkForChangesInOptions(oldOptions, newOptions) {
	for (const newOption of newOptions) {
		...
		if (oldOption.choices || newOption.choices) {
			...
			if (checkForChangeInChoices(oldOption.choices, newOption.choices)) return true;
		}
